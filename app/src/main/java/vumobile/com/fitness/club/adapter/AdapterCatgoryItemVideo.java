...
public class AdapterCatgoryItemVideo extends RecyclerView.Adapter<AdapterCatgoryItemVideo.VideoCatHolder>{

    private List<RelatedContentClass> catgoryItemsClassList;
    private Context mContext;

    public AdapterCatgoryItemVideo(Context context, List<RelatedContentClass> list){
        this.mContext = context;
        this.catgoryItemsClassList = list;
    }

    @Override
    public VideoCatHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.row_video_cat_content, parent, false);
        return new VideoCatHolder(view);
    }

    @Override
    public void onBindViewHolder(VideoCatHolder holder, int position) {

        RelatedContentClass catgoryItemsClass = catgoryItemsClassList.get(position);
